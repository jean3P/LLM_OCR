...
def calculate_label_correctness_and_mean_cer(json_file_path):
    ...
    for entry in data:
        ...
        if entry['Check Label - Using OCR errors']['is_correct_1'] == "Yes":
            check_label_1_correct_count += 1
        else:
            check_label_1_incorrect_count += 1

        # Calculate correctness counts for Check Label-2
        if entry['Check Label - Using OCR examples']['is_correct_2'] == "Yes":
        ...
        check_label_1_cer_values.append(entry['Check Label - Using OCR errors']['cer'])
        check_label_2_cer_values.append(entry['Check Label - Using OCR examples']['cer'])
...
json_file_path = os.path.join(automated_resuts_experiments, 'final_exp_8_v4.json')
