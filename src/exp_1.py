...
def SelectBestSentence(ocr_sentence, mistral_sentence, pipe):
    ...
    prompt = (f"<s>[INST] You are provided with two sentences: One comes after applying an OCR model and the other "
              f"after using an LLM to correct the sentence produced by the OCR. Your task is to select the best "
              f"sentence that corrects the OCR error:\n"
              ...
              f"\nInputs:\n"
              f"    OCR sentence: {ocr_sentence}\n"
              f"    Mistral sentence: {mistral_sentence}\n"
              f"\nExample:\n"
              f"    OCR sentence: Hello htere\n"
              f"    Mistral sentence: Hello there\n"
...
## Example usage
evaluate_test_data_mistral7B(loaded_data, mistral_pipe, 'final_exp_1.json', True)
