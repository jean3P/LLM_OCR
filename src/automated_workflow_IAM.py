...
from src.utils.constants import outputs_path, automated_resuts, results_test_trocr, TOKEN, iam_outputs_path


def evaluate_with_mistral(mistral_model_name, loaded_data, name_mistral, short_system):
    with torch.no_grad():
        ...
        mistral_pipe = pipeline("text-generation", model=mistral_model, tokenizer=mistral_tokenizer,
                                batch_size=10)
        # Assuming `evaluate_test_data_mistral7B` is defined elsewhere and properly handles evaluation
        evaluate_test_data_mistral7B(loaded_data, mistral_pipe, name_mistral, short_system)

    # Free up memory
    del mistral_model
    del mistral_tokenizer
    del mistral_pipe
    clear_cuda_cache()  # Clear CUDA cache if using GPU
...
def automate_workflow(start_percentage=25, increments=25, max_iterations=3, training_data_path='', valid_data_path='',
    ...
    for iteration in range(max_iterations):
        ...
        test_final = os.path.join(iam_outputs_path, 'test', 'testing_seq_data.json')
        ...
        if not file_exists(name_mistral_1_final, automated_resuts):
            ...
            evaluate_with_mistral(mistral_model_name, loaded_data, name_mistral_1_final, True)
        ...
        if start_percentage < 100:
            ...
            if not file_exists(name_mistral_1, automated_resuts):
                ...
                evaluate_with_mistral(mistral_model_name, loaded_data, name_mistral_1, False)
            ...
            if not file_exists(name_mistral_3, automated_resuts):
                loaded_data = load_from_json(results_path_from_ocr)
                evaluate_with_mistral(mistral_model_name, loaded_data, name_mistral_3, True)
        start_percentage = start_percentage + increments


if __name__ == "__main__":
    training = os.path.join(iam_outputs_path, 'train', 'training_seq_data.json')
    valid = os.path.join(iam_outputs_path, 'valid', 'validation_seq_data.json')
