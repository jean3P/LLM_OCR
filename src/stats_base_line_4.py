...
def calculate_stats(data):
    ...
    for entry in data:
        ...
        total_prompt_correcting_cer += entry["Prompt OCR correcting"]["cer"]
        total_confidence += entry["Prompt OCR correcting"]["confidence"]
        if entry["Prompt OCR checking"]["response"] == "Yes":
            yes_count += 1
        elif entry["Prompt OCR checking"]["response"] == "No":
            no_count += 1
