...
def find_top_3_similar_words(word, train_set_lines):
    def split_suggestion(suggestion, original):
        if original.endswith('-') and suggestion.endswith('-'):
            return suggestion
        if original.endswith('-') and not suggestion.endswith('-'):
            if len(original) > 1 and original[-2].isalpha():
                # It's a cut-off word (e.g., "exam-")
                return suggestion[:len(original) - 1] + '-'
            else:
                # Not a cut-off word but ends with a hyphen
                return suggestion + '-'
        return suggestion
...
def detect_duplicated_words_prompt(text_line, pipe, mistral_tokenizer):
    system_prompt = (
        f"Act as an 18th-century document analyst specializing in OCR error identification. "
        f"Your task is to analyse the structure of each word and to respond with 'Yes' or 'No' to the question: "
        f"Does the given text line contain duplicated words in the text line? "
        f"\n\n## Guidelines:"
        f"\n1. Check if any word is immediately duplicated in the text line"
        f"\n2. Do not consider duplicated words if they appear separated by other non duplicated words"
        f"\n3. Not all text lines have duplicated words; therefore, analyze them carefully"
        f"\n4. Typically, OCR duplication errors occur when a word is duplicated immediately after itself"
        f"\n[INST] Does the text line contain duplicated words? The text line is: '{text_line}' [/INST]"
        f"\nThen the output is:"
    )

    tokens_prompt = count_tokens(system_prompt, mistral_tokenizer) + 5
    response = calculate_pipe(pipe, system_prompt, tokens_prompt, 1)
    raw_response = response[0]['generated_text']
    ...
    if json_output_marker in raw_response:
        json_output = raw_response.split(json_output_marker)[-1].strip()
        # print("JSON output:", json_output)  # Debug print

        if "Yes" in json_output:
            response = "Yes"
            # print(f"Logged duplicated words detection for: '{text_line}'")  # Additional debug print
        elif "No" in json_output:
            response = "No"
        else:
            response = "No"
            # print("No valid response found.")  # Additional debug print
    else:
        response = "No"
        # print("No valid JSON output found.")  # Additional debug print
...
def find_immediate_repeated_words(text_line):
    ...
    repeated_words = re.findall(r'\b(\w+)\b[\s\W]+\1\b', text_line, flags=re.IGNORECASE)
...
def correct_duplicated_words_prompt(text_line, pipe, mistral_tokenizer):
    ...
    duplicated_words = find_immediate_repeated_words(text_line)

    if not duplicated_words:
        # If no duplicated words are found, return the original text line
        return text_line

    # If duplicated words are found, proceed with correction
    duplicated_words_part = "\n".join(
        f"Duplicated word: {word}" for word in duplicated_words
    )
